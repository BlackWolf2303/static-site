...
NodeList.prototype.forEach = Array.prototype.forEach,
    function() {
        var Masonry = function(gridElement, options) {
            this.el = gridElement, this.el.masonry = this, this.options = options || {}, this.childSelector = this.el.getAttribute("data-child-selector") || "", this.children = this.childSelector ? this.el.querySelectorAll(this.childSelector) : this.el.children, this.filterSelector = "", this.filteredSelection = [], this.filterButtonsWrapper = this.el.getAttribute("data-filter") || "", this.originalTransition = "transform .3s ease, opacity .3s ease", this.currentTransition = "0s", this.css3 = !0, this.columns = parseInt(this.el.getAttribute("data-columns")) || 3, this.gutter = parseInt(this.el.getAttribute("data-gutter")) || 0, this.space = this.el.offsetWidth == document.body.clientWidth ? "around" : "between", this.cellRatio = eval(this.el.getAttribute("data-cell-ratio")) || 5 / 4, this.mobileWidth = parseInt(this.el.getAttribute("data-mobile-width")) || 768, this.colMinWidth = parseInt(this.el.getAttribute("data-col-min-width")) || 350, this.init()
        };
        Masonry.prototype.init = function() {
            var t = this;
            this.el.style.position = "relative", this.el.style.overflow = "hidden", imagesLoaded(t.el, t.update.bind(t)), this.filterButtonsWrapper && this.initFilterButtons();
            var i;

            function e() {
                t.currentTransition = "0s", cancelAnimationFrame(i), i = requestAnimationFrame(t.update.bind(t))
            }
            window.addEventListener("resize", e);
            var s = document.createElement("iframe");
            s.id = "resizeListener", s.style.cssText = "position:absolute;pointerEvents:none;opacity:0;height:0;border:0;width:100%;", s.onload = function() {
                s.contentWindow.addEventListener("resize", e)
            }, document.body.appendChild(s)
        }, Masonry.prototype.update = function() {
            this.gw = this.el.offsetWidth;
            var t = window.innerWidth > this.mobileWidth ? this.columns : 1;
            if (!this.options.gridMode) {
                var i = Math.floor(this.gw / this.colMinWidth) || 1;
                t = Math.min(i, t)
            }
            for (this.arr = []; t > 0; t--) this.arr.push(0);
            this.gutterSum = (this.arr.length + ("around" == this.space ? 1 : -1)) * this.gutter, this.iw = (this.gw - this.gutterSum) / this.arr.length;
            for (var e = 0; e < this.children.length; e++) {
                var s = this.children[e],
                    r = this.arr,
                    n = 1;
                if (s.style.position = "absolute", s.style.transition = this.currentTransition, this.filteredSelection.length && -1 === [].indexOf.call(this.filteredSelection, s)) s.style.opacity = 0, s.style.pointerEvents = "none";
                else {
                    if (this.options.gridMode) {
                        n = parseInt(s.getAttribute("data-w")) || 1;
                        var o = parseInt(s.getAttribute("data-h")) || 1;
                        if (s.classList.contains("wide") && (n = 2), s.classList.contains("tall") && (o = 2), n > this.arr.length && (n = this.arr.length), n > 1) {
                            r = [];
                            for (var a = 0; a <= this.arr.length - n; a++) {
                                for (var l = [], h = a; h < a + n; h++) l.push(this.arr[h]);
                                r.push(Math.max.apply(null, l))
                            }
                        }
                    }
                    var c = Math.min.apply(null, r),
                        d = r.indexOf(c),
                        u = d * this.iw + (d + ("around" == this.space ? 1 : 0)) * this.gutter,
                        p = c || "around" == this.space ? c + this.gutter : c;
                    s.style.width = n * this.iw + (n - 1) * this.gutter + "px", this.options.gridMode && (s.style.height = o * this.iw / this.cellRatio + (o - 1) * this.gutter + "px"), s.style.opacity = 1, s.style.pointerEvents = "auto", this.css3 ? s.style.transform = "translate3d(" + u + "px, " + p + "px, 0)" : (s.style.left = u + "px", s.style.top = p + "px");
                    for (a = d; a < d + n; a++) this.arr[a] = p + parseFloat(s.getBoundingClientRect().height)
                }
            }
            this.el.style.height = Math.max.apply(null, this.arr) + ("around" == this.space ? this.gutter : 0) + "px"
            ...
            var t = this,
                i = document.querySelector(t.filterButtonsWrapper);
            i && i.querySelectorAll("[data-filter]").forEach(function(e) {
                var s = e.getAttribute("data-filter");
                e.addEventListener("click", function(r) {
                    if (r.preventDefault(), !e.classList.contains("is-active")) {
                        var n = i.querySelector(".is-active");
                        n && n.classList.remove("is-active"), e.classList.add("is-active"), t.filter(s)
                    }
                })
            })
...
var buildElement = function(t) {
    ...
    PinchZoom = function(t, i) {
        this.el = t, this.zoomFactor = 1, this.lastScale = 1, this.offset = {
            x: 0,
            y: 0
        }, this.options = Object.assign({}, this.defaults, i), this.setupMarkup(), this.bindEvents(), this.update(), this.enable()
    },
    sum = function(t, i) {
        return t + i
    },
    isCloseTo = function(t, i) {
        return t > i - .01 && t < i + .01
    };
PinchZoom.prototype = {
    defaults: {
        tapZoomFactor: 2,
    ...
    handleZoom: function(t, i) {
        var e = this.getTouchCenter(this.getTouches(t)),
            n = i / this.lastScale;
        this.lastScale = i, this.nthZoom += 1, this.nthZoom > 3 && (this.scale(n, e), this.drag(e, this.lastZoomCenter)), this.lastZoomCenter = e
    },
    ...
    handleDoubleTap: function(t) {
        ...
        this.hasInteraction || (n > e && (i = this.getCurrentZoomCenter()), this.animate(this.options.animationDuration, o, this.swing), triggerEvent(this.el, this.options.doubleTapEventName))
    },
    ...
    drag: function(t, i) {
        i && (this.options.lockDragAxis ? Math.abs(t.x - i.x) > Math.abs(t.y - i.y) ? this.addOffset({
            x: -(t.x - i.x),
            y: 0
        }) : this.addOffset({
            y: -(t.y - i.y),
    ...
    sanitizeOffsetAnimation: function() {
        var t = this.sanitizeOffset(this.offset),
            ...
            e = this.offset.y,
            n = function(n) {
                this.offset.x = i + n * (t.x - i), this.offset.y = e + n * (t.y - e), this.update()
            }.bind(this);
    ...
    getCurrentZoomCenter: function() {
        ...
        return 0 === e && (o = this.container.offsetWidth), 0 === r && (c = this.container.offsetHeight), {
            x: o,
            y: c
        }
    },
            ...
            s = function() {
                if (this.inAnimation) {
                    var a = (new Date).getTime() - o,
                        r = a / t;
                    a >= t ? (i(1), n && n(), this.update(), this.stopAnimation(), this.update()) : (e && (r = e(r)), i(r), this.update(), requestAnimationFrame(s))
                }
            }.bind(this);
    ...
    bindEvents: function() {
        var t = this;
        detectGestures(this.container, this), window.addEventListener("resize", this.update.bind(this)), Array.from(this.el.querySelectorAll("img")).forEach(function(i) {
...
var detectGestures = function(t, i) {
    ...
    t.addEventListener("touchstart", function(t) {
        i.enabled && (l = !0, (n = t.touches.length) > 1 && f(t), function(t) {
            var s = (new Date).getTime();
            if (n > 1 && (o = null), s - o < 300) switch (setTimeout(function() {
                f(t)
            }), i.handleDoubleTap(t), e) {
                case "zoom":
                    i.handleZoomEnd(t);
                    break;
                case "drag":
...
! function($) {
    $.fn.elData = function(e, t) {
            ...
            var a = this;
            if (t.navigation && (this.navItems = {}, e = [].map.call(document.querySelectorAll(e), function(e) {
                    var t = e.getAttribute("href");
                    return !t.match(/^#.+/) || (a.navItems[t] = e, t)
                }).join(",")), e && "string" == typeof e) this.nodes = document.querySelectorAll(e);
            ...
            this.options.onDisappear && (this.options.once = !1), this.sequenceTimeout = null, this.sequenceArray = [], (this.options.onAppear || this.options.onDisappear) && [].forEach.call(a.nodes, function(e) {
                a.listenTo(e)
            ...
            var t = this,
                ...
                i = function() {
                    cancelAnimationFrame(o), o = requestAnimationFrame(function() {
                        ...
                        o.top < window.innerHeight - t.options.offsetBottom && o.bottom > t.options.offsetTop ? (t.options.once && window.removeEventListener("scroll", i), t.options.onAppear && !a && (t.options.sequenceDelay ? (t.sequenceArray.push(e), clearTimeout(t.sequenceTimeout), t.sequenceTimeout = setTimeout(function() {
                            t.sequenceArray.forEach(function(e, a) {
                                setTimeout(function() {
                                    t.options.onAppear.call(e)
                                }, t.options.sequenceDelay * a)
                            }), t.sequenceArray = []
                        }, 100)) : t.options.onAppear.call(e)), a = !0) : a && (t.options.onDisappear && t.options.onDisappear.call(e), a = !1)
                    })
                };
            ...
            var e = this,
                ...
                o = function() {
                    ...
                    for (var o = 0, i = e.nodes.length; o < i; o++) {
                        var n = e.nodes[o];
                        if (!(n.getBoundingClientRect().top < window.innerHeight / 2)) break;
                        t.push(n)
                    }
                    var s = t[t.length - 1];
                    a != s && (e.options.active && e.options.active.call(s), e.options.navigation && (a && e.navItems["#" + a.id].classList.remove("active"), e.navItems["#" + s.id].classList.add("active")), a = s)
                };
        ...
        function() {
            if (document.body.classList.contains("sticky-header")) {
                ...
                raf("scroll", function() {
                    ...
                    t ? a ? (document.body.classList.add("scrolling-down"), e = window.pageYOffset) : e - window.pageYOffset > 30 && (document.body.classList.remove("scrolling-down"), e = window.pageYOffset) : (document.body.classList.remove("scrolling-down"), e = window.pageYOffset)
                })
            }
        }(), [].forEach.call(document.querySelectorAll("[data-background], [data-poster]"), function(e) {
            if (!e.classList.contains("background-parallax")) {
                var t = e.getAttribute("data-background") || e.getAttribute("data-poster");
                t && (t.match("^rgb") || t.match("^#") ? e.style.backgroundColor = t : e.classList.contains("lazy-load") ? new ScrollListener(e, {
                    onAppear: function() {
            ...
            i.classList.add("content-overlay"), i.style.backgroundColor = a, i.style.opacity = o, e.insertBefore(i, e.firstChild)
            ...
            e.querySelectorAll(".reveal-section").forEach(function(e, t) {
                if (0 != t) {
                    ...
                    raf("scroll", function() {
                        ...
                        t < window.innerHeight && t > 0 ? (e.classList.add("is-fixed"), a.style.opacity = 1 - t / window.innerHeight) : (e.classList.remove("is-fixed"), a.style.opacity = 1)
                    })
                }
            })
            ...
            var e, t, a = $(this),
                o = a.attr("data-src");
            o.indexOf("youtube.com/") > -1 ? (t = o.split("watch?v=")[1], e = '<iframe src="https://www.youtube.com/embed/' + t + '?autoplay=1" frameborder="0" allowfullscreen></iframe>') : o.indexOf("vimeo.com/") > -1 ? (t = o.split(".com/")[1], e = '<iframe src="https://player.vimeo.com/video/' + t + '?autoplay=1&title=0&byline=0&portrait=0&transparent=0" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>') : e = '<video src="' + o + '" autoplay controls></video>', a.one("click", function() {
            ...
            }, e.find(".video-background").length && (a.on.slideChangeTransitionStart = function() {
            ...
            }, a.on.init = a.on.slideChangeTransitionEnd), e.find(".lightbox-link:not(.overlay)").each(function() {
                this.classList.add("noswipe")
            }), $(window).on("load", function() {
                "auto" == a.slidesPerView && navigator.userAgent.indexOf("Chrome") > -1 && e.hide().show(0), new Swiper(e[0], a)
            })
        }), $(window).on("load", function() {
            ...
            $(window).on("resize", function() {
                $(".justified a").css("opacity", 0), i && clearTimeout(i), i = setTimeout(o, 200)
            })
        }), [].forEach.call(document.querySelectorAll('a[href^="#"]:not([href="#"])'), function(e) {
            ...
            if (t[0].checkValidity()) {
                ...
                t.data("mailto") && (a.mailto = t.data("mailto")), t.data("subject") && (a.subject = t.data("subject"));
                var o = [];
                t.find("[name]").each(function() {
                    var e = $(this).attr("name");
                    if (!(o.indexOf(e) > -1)) {
                        var t = $(this).val();
                        null !== t && void 0 !== t && "" !== t && ("radio" == $(this).attr("type") && (t = $("[name=" + e + "]:checked").val()), "checkbox" == $(this).attr("type") && (t = $(this).prop("checked") ? "Yes" : "No"), "object" == typeof t && (t = t.join("; ")), a.items[e] = t, ["name", "email", "subject"].indexOf(e) > -1 && (a[e] = t), o.push(e))
                    }
                }), $.ajax({
                    type: "POST",
                    url: t.attr("action"),
                    data: a,
                    success: function(e) {
                        $(".mail-form-message").removeClass("d-none").html(e).slideDown(500), -1 == e.indexOf("error") && t.find("[name]:not([type=radio])").val("")
                    },
                    error: function() {
                        $(".mail-form-message").removeClass("d-none").html('<p class="error">Something went wrong, try again!</p>').slideDown("slow")
                    }
                })
    ...
    gmaps_api_key && $("body").append('<script src="https://maps.googleapis.com/maps/api/js?key=' + gmaps_api_key + '&callback=initMaps"><\/script>'), window.initMaps = function() {
        $(".google-map").each(function() {
            var e = $(this);
            e.addClass("ready");
            var t = e.data("latitude") || 51.5255069,
                a = e.data("longitude") || -.0836207,
                o = e.data("zoom") || 14,
 ...
 */
! function(t) {
    var e = localStorage.getItem("navcolor") || "dark",
        o = localStorage.getItem("navtype") || "floating",
        a = t('<div class="demo-panel">');
    a.html('\t\t<div class="panel-handle"><i class="fa fa-cogs"></i></div>\t\t<label>Navigation Color</label>\t\t<button class="button navcolor-light style-3">Light</button>\t\t<button class="button navcolor-dark">Dark</button>\t\t<label>Navigation Type</label>\t\t<select>\t\t\t<option value="floating">Header+Footer (floating)</option>\t\t\t<option value="fixed">Header+Footer (fixed)</option>\t\t\t<option value="only_header">Header</option>\t\t</select>\t').appendTo("body"), t("<style>").html("\t\t.demo-panel {\t\t\tposition:relative;\t\t\twidth:272px;\t\t\tbackground:white;\t\t\tposition:fixed;\t\t\tright:0;\t\t\ttop:calc(50% - 120px);\t\t\tpadding:20px 26px 28px;\t\t\tborder: 1px solid rgba(0, 0, 0, .15);\t\t\tz-index:999;\t\t\ttransform: translateX(100%);\t\t\ttransition: .2s;\t\t}\t\t.demo-panel:hover {\t\t\ttransform: translateX(0);\t\t}\t\t.panel-handle {\t\t\tposition:absolute;\t\t\tright:100%;\t\t\ttop:-1px;\t\t\tbackground:white;\t\t\twidth:40px;\t\t\theight:40px;\t\t\tfont-size: 22px;\t\t\ttext-align: center;\t\t\tline-height:40px;\t\t\tborder: 1px solid rgba(0, 0, 0, .15);\t\t\tborder-right:0;\t\t\tcursor: pointer;\t\t}\t\t.demo-panel label {\t\t\tfont-weight: bold;\t\t\tdisplay: block;\t\t}\t\t.demo-panel button {\t\t\theight: 34px;\t\t\tline-height: 31px;\t\t\tpadding: 0 20px;\t\t}\t\t.demo-panel select {\t\t\theight: 34px;\t\t\tline-height: 31px;\t\t\tpadding: 0 20px;\t\t\tmargin-bottom: 0;\t\t}\t").appendTo("body"), "light" == e && (t("body").addClass("light-navigation"), a.find(".navcolor-light").removeClass("style-3").siblings("button").addClass("style-3")), a.on("click", "button", function(o) {
            o.preventDefault(), t(this).removeClass("style-3").siblings("button").addClass("style-3"), t(this).hasClass("navcolor-light") ? (e = "light", t("body").addClass("light-navigation")) : (e = "dark", t("body").removeClass("light-navigation")), localStorage.setItem("navcolor", e), document.body.classList.contains("scrolling-down") && t("html, body").animate({
                scrollTop: window.pageYOffset - 100
            }, 300)
        }),
        function(e) {
            var o = t("body");
            o.removeClass("sticky-header sticky-footer fixed-header fixed-footer"), "floating" == e ? o.addClass("sticky-header sticky-footer") : "fixed" == e ? o.addClass("fixed-header fixed-footer") : o.addClass("fixed-header")
        }(o), a.find('option[value="' + o + '"]').attr("selected", ""), a.on("input", "select", function() {
            localStorage.setItem("navtype", this.value), window.location.reload()
        });
    t(window).on("load", function() {
        document.querySelectorAll(".background-parallax").forEach(function(t) {
            t.parallaxInstance && t.parallaxInstance.setSize()
        })
    })
}(jQuery);